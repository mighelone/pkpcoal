...
class CPD_Result(object):
    """ Reads the CPD input and saves the values in one array. 
        ...
        The results include the yields and the rates. The rates were calculated 
        using a CDS. This class also contains the dictionaries for the columns 
        in the array - the name of the species. These dictionaries are CPD-Version 
        dependent and the only thing which has to be changed for the case of a new r
        elease of CPD with a new order of species in the result files. """
    ...
    def __init__(self,FilePath):
        ...
        # self.__yields=np.zeros((int(len(yields1[:,0])), 11), order='F') 
        # self.__yields[:,0:2]  = yields1[:,0:2]  # 0=Time 1=Temp
        ...
        #     self.__rates[0,col] = (self.__yields[1,col] - self.__yields[0,col]) /
        ...
        #
        # print '\nimported data-fields, size(rows,columns): ',
        #         self.__yields.shape,'\n'
        # self._setYieldsCols()

    def _setYieldsCols(self):
        """ Yields2Cols: updated for FG-DVC Version 8.2.2, 
            one comment: 'Char_and_Ash' is actual the whole coal mass, 
            but this part is named in the FG-DVC output file that way """
        #TODO GO since it seems to be ordered use a list here
        self.Yields2Cols = {'Time':0, 
                            'Temp':1,
                            'Tar':2,
                            'Gas':3,
                            'Solid':4,
