...
def callbackKoba(X):
    global Nfeval
    print '{0:4d}\t{1: 3.6f}\t{2: 3.6f}\t{3: 3.6f}\t{4: 3.6f}\t{4: 3.6f}\t{5: 3.6f}\t{6: 3.6f}\t{7: 3.6f}'.format(Nfeval, ErrorGl, X[0], X[1], X[2], X[3], X[4], X[5])
    Nfeval += 1
#
...
class TwoPointEstimator(object):
    ...
    def estimate(self, fgdvc, Species, PointLocation):
        # first part: calculates a list of VM_s and the precursor values
        t = fgdvc.Time()
...
class LeastSquarsEstimator(object):
    ...
    def __init__(self):
        ...
        # Number of maximum iterations used for the self.improve_?
        # functions
        self.PreMaxIter = 50
    ...
    def improve_E(self, fgdvc, model, t, T, Parameter_Vector, Name,):
        ...
        def E_adjust(ActivationEnergie):
            model.setParamVector(
                [Parameter_Vector[0], Parameter_Vector[1], ActivationEnergie])
            print model.ParamVector()
            uDot = fgdvc.Rate(Name)
            vDot = model.calcRate(fgdvc, t, T, Name)
            return sum((uDot - vDot)**2)
        # InitialCondition for Optimzation
        IC_ActEner = Parameter_Vector[2]
        # Optimizing
        # shifts the rates curves by the Activation Energy together
        Optimized_E1 = fmin(
            E_adjust, IC_ActEner, ftol=self.Pre_Tolerance, maxiter=self.PreMaxIter)
        # reformate, because Vec has shape: [0.1, 5.0, array([
        # 26468.75])]
    ...
    def improve_a(self, fgdvc, model, t, T, Parameter_Vector, Name):
        ...
        def a_adjust(aPre):
            model.setParamVector(
                [aPre, Parameter_Vector[1], Parameter_Vector[2]])
            print model.ParamVector()
            uDot = fgdvc.Rate(Name)
            vDot = model.calcRate(fgdvc, t, T, Name)
            return sum((uDot - vDot)**2)
        Optimized_a1 = fmin(
            a_adjust, IC_a[0], ftol=self.Pre_Tolerance, maxiter=self.PreMaxIter)
    ...
    def estimate_T(self, fgdvc_list, model, Parameter_Vector, Name, preLoopNumber=0):
        ...
        # line index, time, column index: runned case
        t = [[] for i in range(len(fgdvc_list))]
        # line index, time, column index: runned case
        dt = [[] for i in range(len(fgdvc_list))]
        T = [[] for i in range(len(fgdvc_list))]
        # line index, time, column index: runned case
        u = [[] for i in range(len(fgdvc_list))]
        # line index, time, column index: runned case
        uDot = [[] for i in range(len(fgdvc_list))]
        # line index, time, column index: runned case
        v = [[] for i in range(len(fgdvc_list))]
        # line index, time, column index: runned case
        vDot = [[] for i in range(len(fgdvc_list))]
        # this are the main arrays, containing sublists
        # in the following, each subelement in every list is indicated
        # with an underscore, see next loop
        ...
        w0 = GlobalOptParam.ScaleFactor * self.a0 / \
            ((max((fgdvc_list[0].Yield(Name))) -
              min((fgdvc_list[0].Yield(Name))))**2)
        w1 = GlobalOptParam.ScaleFactor * self.a1 / \
            (max(((fgdvc_list[0].Rate(Name)))**2))
        print 'start gradient based optimization, species:', fgdvc_list[0].SpeciesName(Name)
        # use logaritmic preexp factor
        parameter = np.copy(Parameter_Vector)
        if model._modelName is 'Kobayashi':
            parameter[0] = np.log(parameter[0])  # A1
            parameter[2] = np.log(parameter[2])  # A2

        #
        def LeastSquareFunction(Parameter0):
            """ The function to optimize. Calculates the LS deviation for rates and yields."""
            # adds the final yield to the vector:
            Parameter = np.copy(Parameter0)
            if model._modelName is 'Kobayashi':
                Parameter[0] = np.exp(Parameter[0])
                Parameter[2] = np.exp(Parameter[2])
            ...
            for runnedCaseNr in range(len(fgdvc_list)):
                v_ = model.calcMass(fgdvc_list[runnedCaseNr], t[
                                    runnedCaseNr], T[runnedCaseNr], Name)
            ...
            if self.selectedOptimizer == 'leastsq':
                ...
                for runnedCaseNr in range(len(fgdvc_list)):
                    # the rate term
                    Dot2_ = w1 * \
                        (((uDot[runnedCaseNr] - vDot[runnedCaseNr])
                          ** 2) * dt[runnedCaseNr])
                    # the yield term
                    Dot1_ = w0 * \
                        (((u[runnedCaseNr] - v[runnedCaseNr])**2)
                         * dt[runnedCaseNr])
                    # makes an array, containing both, the rates and
                    # yields
                ...
                for runnedCaseNr in range(len(fgdvc_list)):
                    ...
                    deltaYield2 = (
                        max(u[runnedCaseNr]) - min(u[runnedCaseNr]))**2.
                    Error += np.sum(errori) / ntime * \
                        self.a0 / deltaYield2
                    errori = (uDot[runnedCaseNr] -
                              vDot[runnedCaseNr])**2
                    deltaRate2 = (
                        max(uDot[runnedCaseNr]) - min(uDot[runnedCaseNr]))**2.
                    Error += np.sum(errori) / ntime * \
                        self.a1 / deltaYield2
        ...
        if self.selectedOptimizer == 'fmin':
            if model._modelName is 'Kobayashi':
                callback = callbackKoba
                callback = None
            else:
                callback = None
            OptimizedVector = fmin(LeastSquareFunction, parameter, ftol=self.Fit_Tolerance,
                                   maxiter=self.MaxIter, disp=True, callback=callback)  # calculates optimized vector
            ...
            OptimizedVector = fmin_cg(
                LeastSquareFunction, parameter, gtol=self.Fit_Tolerance, maxiter=self.MaxIter)
            # saves now final deviation
            self.FinalDeviation = LeastSquareFunction(OptimizedVector)
            # appends final yield
            if self.FinalY != False:
            ...
            # calculates optimized vector
            OptimizedVector = fmin_ncg(
                LeastSquareFunction, parameter, avextol=self.Fit_Tolerance)
            # saves now final deviation
            self.FinalDeviation = LeastSquareFunction(OptimizedVector)
            # appends final yield
            if self.FinalY != False:
            ...
            # calculates optimized vector
            OptimizedVector = fmin_slsqp(
                LeastSquareFunction, parameter, acc=self.Fit_Tolerance)
            # saves now final deviation
            self.FinalDeviation = LeastSquareFunction(OptimizedVector)
            # appends final yield
            if self.FinalY != False:
                ...
                OptimizedVector.append(self.FinalY)
                OptimizedVector = np.array(OptimizedVector)
        elif self.selectedOptimizer == 'brute':
            ranges = (
                (GlobalOptParam.EvAKobMin[0],
                 GlobalOptParam.EvAKobMax[0]),
                (GlobalOptParam.EvAKobMin[1],
                 GlobalOptParam.EvAKobMax[1]),
                (GlobalOptParam.EvAKobMin[2],
                 GlobalOptParam.EvAKobMax[2]),
                (GlobalOptParam.EvAKobMin[3],
                 GlobalOptParam.EvAKobMax[3]),
                (GlobalOptParam.EvAKobMin[4],
                 GlobalOptParam.EvAKobMax[4]),
                (GlobalOptParam.EvAKobMin[5],
                 GlobalOptParam.EvAKobMax[5])
            )
            # calculates optimized vector
            OptimizedVector = brute(
                LeastSquareFunction, ranges=ranges, finish=fmin, Ns=5)
            # saves now final deviation
            self.FinalDeviation = LeastSquareFunction(OptimizedVector)
            # appends final yield
            if self.FinalY != False:
            ...
            OptimizedVector = leastsq(LeastSquareFunction, Parameter_Vector,
                                      ftol=self.Fit_Tolerance, maxfev=self.MaxIter)  # calculates optimized vector
        ...
        if model._modelName is 'Kobayashi':
            OptimizedVector[0] = np.exp(OptimizedVector[0])
            OptimizedVector[2] = np.exp(OptimizedVector[2])
...
class GlobalOptimizer(object):
    ...
    def __init__(self, localOptimizer, KineticModel, Fit_one_runObj):
        ...
        # collects the final results of the local minima.
        self.AllParameterList = []
    ...
    def GenerateOptima(self, Species, IndexListofParameterToOptimize, ArrayOfRanges, ListNrRuns):
        """This method makes a several number of runs and
        returns the Parameter having the lowest deviation of all local minima. The
        list contains 3 or 4 parameters. If e.g., the second and the third Parameter have to be optimized: IndexListofParameterToOptimize=[1,2]. If e.g. the range of the second is 10000 to 12000 and for the third 1 to 5,  ArrayOfRanges=[[10000,12000],[1,5]]. When ListNrRuns is e.g. [0,10,5,0] the the second Parameter will be optimizted eleven times between 10000 and 12000, the third six times between 1 and 5. Attention, the number of runs grows by NrRuns1*NrRuns2*NrRuns3*NrRuns4, which can lead to a very large number needing very much time!"""
        ...
        for i in range(len(IndexListofParameterToOptimize)):
            Parameter[IndexListofParameterToOptimize[
                i]] = ArrayOfRanges[i][0]
        self.setParamList(Parameter)
        if len(self.__ParamList) == 4:
            for i in range(ListNrRuns[0] + 1):
                if ListNrRuns[0] != 0:
                    self.__ParamList[0] = ArrayOfRanges[IndexListofParameterToOptimize.index(0)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(0)][
                        1] - ArrayOfRanges[IndexListofParameterToOptimize.index(0)][0]) * ((float(i)) / ListNrRuns[0])
                for j in range(ListNrRuns[1] + 1):
                    if ListNrRuns[1] != 0:
                        self.__ParamList[1] = ArrayOfRanges[IndexListofParameterToOptimize.index(1)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(1)][
                            1] - ArrayOfRanges[IndexListofParameterToOptimize.index(1)][0]) * ((float(j)) / ListNrRuns[1])
                    for k in range(ListNrRuns[2] + 1):
                        if ListNrRuns[2] != 0:
                            self.__ParamList[2] = ArrayOfRanges[IndexListofParameterToOptimize.index(2)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(2)][
                                1] - ArrayOfRanges[IndexListofParameterToOptimize.index(2)][0]) * ((float(k)) / ListNrRuns[2])
                        for l in range(ListNrRuns[3] + 1):
                            if ListNrRuns[3] != 0:
                                self.__ParamList[3] = ArrayOfRanges[IndexListofParameterToOptimize.index(3)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(3)][
                                    1] - ArrayOfRanges[IndexListofParameterToOptimize.index(3)][0]) * ((float(l)) / ListNrRuns[3])
                            #
                            self.KinModel.setParamVector(self.LocOpt.estimate_T(
                                self.FitInfo, self.KinModel, self.__ParamList, Species))
                            DevList.append(self.LocOpt.Deviation())
                            ParamArray.append(
                                self.KinModel.ParamVector())
        ####
                    ...
                    for k in range(ListNrRuns[2] + 1):
                        ...
                        for l in range(ListNrRuns[3] + 1):
                            ...
                            for m in range(ListNrRuns[4] + 1):
                                if ListNrRuns[4] != 0:
                                    self.__ParamList[4] = ArrayOfRanges[IndexListofParameterToOptimize.index(4)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(4)][
                                        1] - ArrayOfRanges[IndexListofParameterToOptimize.index(4)][0]) * ((float(m)) / ListNrRuns[4])
                                for n in range(ListNrRuns[5] + 1):
                                    if ListNrRuns[5] != 0:
                                        self.__ParamList[5] = ArrayOfRanges[IndexListofParameterToOptimize.index(5)][0] + (ArrayOfRanges[IndexListofParameterToOptimize.index(5)][
                                            1] - ArrayOfRanges[IndexListofParameterToOptimize.index(5)][0]) * ((float(n)) / ListNrRuns[5])
                            #
                            self.KinModel.setParamVector(self.LocOpt.estimate_T(
                                self.FitInfo, self.KinModel, self.__ParamList, Species))
                            DevList.append(self.LocOpt.Deviation())
