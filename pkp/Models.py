...
class Model(object):
    ...
    def pltYield(self, fgdvc_list, xValueToPlot, yValueToPlot):
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            plt.plot(fgdvc_list[runnedCaseNr].Yield(xValueToPlot), fgdvc_list[
                     runnedCaseNr].Yield(yValueToPlot))
        ...
        plt.grid()
        if oSystem == 'Linux':
            plt.savefig('Result/' + 'Yields_' + yValueToPlot +
                        'VS' + xValueToPlot + '.pdf', format='pdf')
    ...
    def pltRate(self, fgdvc_list, xValueToPlot, yValueToPlot):
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            plt.plot(fgdvc_list[runnedCaseNr].Rate(fgdvc_list[runnedCaseNr].SpeciesIndex(xValueToPlot)), fgdvc_list[
                     runnedCaseNr].Rate(fgdvc_list[runnedCaseNr].SpeciesIndex(yValueToPlot)), label=yValueToPlot)
        if xValueToPlot == 'Time':
            plt.xlabel('t in s')
        if xValueToPlot == 'Temp':
            plt.xlabel('T in K')
        ...
        plt.grid()
        if oSystem == 'Linux':
            plt.savefig('Result/' + 'Rates_' + yValueToPlot +
                        'VS' + xValueToPlot + '.pdf', format='pdf')
        elif oSystem == 'Windows':
            plt.savefig('Result\\' + 'Rates_' + yValueToPlot +
                        'VS' + xValueToPlot + '.pdf', format='pdf')
    ...
    def plot(self, fgdvc_list, Species):
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            ...
            v_ = self.calcMass(fgdvc_list[runnedCaseNr], fgdvc_list[runnedCaseNr].Time(
            ), fgdvc_list[runnedCaseNr].Interpolate('Temp'), Species)
            u.append(u_)
            v.append(v_)
        if type(Species) == int:
        ...
        if SpeciesForTitle == 'Solid':
            for runnedCaseNr in range(len(fgdvc_list)):
                plt.plot(fgdvc_list[runnedCaseNr].Time()[:len(u[runnedCaseNr])], u[
                         runnedCaseNr], '-', color=colors[runnedCaseNr], label=fgdvc_list[0].Name() + ' ' + str(runnedCaseNr))
                plt.plot(fgdvc_list[runnedCaseNr].Time()[:len(v[runnedCaseNr])], v[
                         runnedCaseNr], '--', color=colors[runnedCaseNr], label='fit')
                plt.plot(fgdvc_list[runnedCaseNr].Time()[:len(u[runnedCaseNr])], (1. - u[runnedCaseNr]),
                         '-', color=colors[runnedCaseNr], label='Sum yields' + ' ' + str(runnedCaseNr))
                plt.plot(fgdvc_list[runnedCaseNr].Time()[:len(v[runnedCaseNr])], (
                    1. - v[runnedCaseNr]), '--', color=colors[runnedCaseNr], label='fit')
        else:
            for runnedCaseNr in range(len(fgdvc_list)):
                plt.plot(fgdvc_list[runnedCaseNr].Time()[:len(u[runnedCaseNr])], u[
                         runnedCaseNr], '-', color=colors[runnedCaseNr], label=fgdvc_list[0].Name() + ' ' + str(runnedCaseNr))
        ...
        plt.grid()
        if oSystem == 'Linux':
            plt.savefig('Result/' + fgdvc_list[0].Name(
            ) + '-Fit_result_' + SpeciesForTitle + '_Y.pdf', format='pdf')
            plt.savefig('Result/' + fgdvc_list[0].Name() + '-Fit_result_' +
                        SpeciesForTitle + '_Y.png', dpi=PngResolution, format='png')
        elif oSystem == 'Windows':
            plt.savefig('Result\\' + fgdvc_list[0].Name(
            ) + '-Fit_result_' + SpeciesForTitle + '_Y.pdf', format='pdf')
            plt.savefig('Result\\' + fgdvc_list[0].Name() + '-Fit_result_' +
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            ...
            plt.plot(fgdvc_list[runnedCaseNr].Time(), ur, '-', color=colors[
                     runnedCaseNr], label=fgdvc_list[runnedCaseNr].Name() + ' ' + str(runnedCaseNr))
            w = self.deriveC(fgdvc_list[runnedCaseNr], v[runnedCaseNr])
            plt.plot(fgdvc_list[runnedCaseNr].Time(), w,
                     '--', color=colors[runnedCaseNr], label='fit')
        if type(Species) == int:
            SpeciesForTitle = fgdvc_list[0].SpeciesName(Species)
        if type(Species) == str:
            SpeciesForTitle = Species
        ...
        if oSystem == 'Linux':
            plt.savefig('Result/' + fgdvc_list[0].Name(
            ) + '-Fit_result_' + SpeciesForTitle + '_R.pdf', format='pdf')
            plt.savefig('Result/' + fgdvc_list[0].Name() + '-Fit_result_' +
                        SpeciesForTitle + '_R.png', dpi=PngResolution, format='png')
        elif oSystem == 'Windows':
            plt.savefig('Result\\' + fgdvc_list[0].Name(
            ) + '-Fit_result_' + SpeciesForTitle + '_R.pdf', format='pdf')
            plt.savefig('Result\\' + fgdvc_list[0].Name() + '-Fit_result_' +
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            ...
            if oSystem == 'Linux':
                ...
                resultFile = open('Result/' + fgdvc_list[runnedCaseNr].Name(
                ) + '-Fit_result_' + SpeciesForTitle + '_' + str(runnedCaseNr) + '.out', 'w')
            elif oSystem == 'Windows':
                print fgdvc_list[runnedCaseNr]
                resultFile = open('Result\\' + fgdvc_list[runnedCaseNr].Name(
                ) + '-Fit_result_' + SpeciesForTitle + '_' + str(runnedCaseNr) + '.out', 'w')
                ...
                print 'Models: Operating Platform cannot be specified.'
            resultFile.write(
                '    Time       Temperature    Yields       Rates    Yields(original) Rates(original) \n')
    ...
    def ErrorYield(self, fgdvc, Species):  # calculates avrg Error between curves
        ...
        v = self.calcMass(fgdvc, fgdvc.Time(),
                          fgdvc.Interpolate('Temp'), Species)
    ...
    def mkSimpleResultFiles(self, fgdvc_list, Species):
        ...
        for runnedCaseNr in range(len(fgdvc_list)):
            ...
            if oSystem == 'Linux':
                ...
                print 'Models: Operating Platform cannot be specified.'
            resultFile.write(
                '    Time       Temperature Yields(original) Rates(original) \n')
...
class ConstantRateModel(Model):
    ...
    def __init__(self, InitialParameterVector):
        ...
        # if set to false, the numerical time step corresponding to the
        # outputted by the dtailled model (e.g CPD) is used; define a
        # value to use instead this
        self.constDt = False
    ...
    def calcMass(self, fgdvc, t, T, SpeciesToCalc):
        ...
        if SpeciesToCalc == 'Solid' or SpeciesToCalc == (fgdvc.SpeciesIndex('Solid')):
            v = u_s0 + (u_0 - u_s0) * \
                np.exp(-ParamVector[0] * (t - ParamVector[1]))
        else:
            v = u_s0 * \
                (1. - np.exp(-ParamVector[0] * (t - ParamVector[1])))
...
class ArrheniusModel(Model):
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            # or Name == fgdvc.Yields2Cols['Solid']:
            if Name == 'Solid':
                dmdt_out = - \
                    ParamVec[0] * (T(t)**ParamVec[1]) * \
                    np.exp(-ParamVec[2] / (T(t))) * (m - m_s0)
                # sets values<0 =0.0, otherwise it will further cause
                # problems (nan)
                dmdt_out = np.where(
                    abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
            else:
                dmdt_out = ParamVec[
                    0] * (T(t)**ParamVec[1]) * np.exp(-ParamVec[2] / (T(t))) * (m_s0 - m)
                # sets values<0 =0.0, otherwise it will further cause
                # problems (nan)
                dmdt_out = np.where(
                    abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
        ...
        m_out = sp.integrate.odeint(dmdt, InitialCondition, timeInt,
                                    atol=absoluteTolerance, rtol=relativeTolerance, hmax=self.ODE_hmax)
        if self.constDt == False:
    ...
    def ConvertKinFactors(self, ParameterVector):
        ...
        # does nothing, just to have the same way of use for all
        # notations
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            ...
            if Name == 'Solid':
                ...
                dmdt_out = ParamVec[
                    0] * np.exp(-ParamVec[1] / (T(t))) * (m_s0 - m)
                # sets values<0 =0.0, otherwise it will further cause
                # problems (nan)
                dmdt_out = np.where(
                    abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
...
class ArrheniusModelAlternativeNotation1(ArrheniusModel):
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            # or Name == fgdvc.Yields2Cols['Solid']:
            if Name == 'Solid':
                # -ParamVec[0]*( (T(t)/self.T0)**ParamVec[1] )*np.exp( -ParamVec[2]*(self.T0/(T(t))-1) )*(m)    #IC
                dmdt_out = - \
                    np.exp(ParamVec[0] - ParamVec[1] *
                           (self.T0 / T(t) - 1)) * (m - m_s0)
                # sets values<0 =0.0, otherwise it will further cuase
                # problem(nan)
                dmdt_out = np.where(
                    abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
            else:
                dmdt_out = np.exp(
                    ParamVec[0] - ParamVec[1] * (self.T0 / T(t) - 1)) * (m_s0 - m)
            return dmdt_out
        InitialCondition = [u[0]]
        m_out = sp.integrate.odeint(
            dmdt, InitialCondition, timeInt, atol=absoluteTolerance, rtol=relativeTolerance)
        if self.constDt == False:
            return m_out[:, 0]
        else:  # returns the short, interpolated list (e.g. for PCCL)
            return self._mkInterpolatedRes(m_out[:, 0], time)
    ...
    def ConvertKinFactorsToOwnNotation(self, fgdvc, ParameterVector, Species):
        """
        Converts the standard Arrhenius kinetic factors backk to the
        factors of the own notation.
        """
        time = fgdvc.Time()
...
class ArrheniusModelAlternativeNotation2(ArrheniusModel):
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            # or Name == fgdvc.Yields2Cols['Solid']:
            if Name == 'Solid':
                dmdt_out = -np.exp(self.c * (ParamVec[0] * (1. / T(t) - 1. / self.T_min) - ParamVec[
                                   1] * (1. / T(t) - 1. / self.T_max))) * (m - m_s0)
                # sets values<0 =0.0, otherwise it will further cause
                # problem(nan)
                dmdt_out = np.where(
                    abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
    ...
    def ConvertKinFactors(self, ParameterVector):
        ...
        self.c = 1. / (1. / self.T_max - 1. / self.T_min)
        A = np.exp(-self.c * ParameterVector[0] / self.T_min +
                   self.c * ParameterVector[1] / self.T_max)
    ...
    def ConvertKinFactorsToOwnNotation(self, ParameterVector):
        ...
        if ParameterVector[1] != 0:
            print 'b is not considered in this Notation and set to 0'
        b2 = ((self.T_max / self.c) * np.log(ParameterVector[0]) - (ParameterVector[
              2] * self.T_max) / (self.c * self.T_min)) * (1 - self.T_max / self.T_min)**(-1)
...
class Kobayashi(Model):
    ...
    def __init__(self, InitialParameterVector):
        ...
        self.ODE_hmax = 1.e-2
        self.constDt = False
        # if set to false, the numerical time step corresponding to
        # the outputted by the dtailled model (e.g CPD) is used;
        # define a value to use instead this
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            ...
            k1k2.append(k1 + k2)
            self.__Integral += 0.5 * \
                (tList[-1] - tList[-2]) * (k1k2[-1] + k1k2[-2])
            dmdt_out = (
                (ParamVec[4] * k1 + ParamVec[5] * k2) * np.exp(-self.__Integral))
            # sets values<0 =0.0, otherwise it will further cause
            # problem(nan)
        ...
        InitialCondition = [0]
        m_out = sp.integrate.odeint(
            dmdt, InitialCondition, timeInt, atol=1.e-5, rtol=1.e-4, hmax=1.e-2)
        m_out = m_out[:, 0]
...
class KobayashiPCCL(Model):
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            ...
            k2 = ParamVec[1] * \
                np.exp(-(ParamVec[2] + self.__E2diff) / (T(t)))
            tList.append(t)
            k1k2.append(k1 + k2)
            self.__Integral += 0.5 * \
                (tList[-1] - tList[-2]) * (k1k2[-1] + k1k2[-2])
            dmdt_out = (
                (ParamVec[3] * k1 + self.__alpha2 * k2) * np.exp(-self.__Integral))
            # sets values<0 =0.0, otherwise it will further cause
            # problem(nan)
            dmdt_out = np.where(abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
            return dmdt_out
...
class KobayashiA2(Model):
    ...
    def calcMass(self, fgdvc, time, T, Name):
        ...
        def dmdt(m, t):
            k1 = np.exp(self.c * (ParamVec[0] * (1. / T(
                t) - 1. / self.T_min) - ParamVec[1] * (1. / T(t) - 1. / self.T_max)))
            k2 = np.exp(self.c * (ParamVec[2] * (1. / T(
                t) - 1. / self.T_min) - ParamVec[3] * (1. / T(t) - 1. / self.T_max)))
            self.tList.append(t)
            self.k1k2.append(k1 + k2)
            self.Integral += 0.5 * \
                (self.tList[-1] - self.tList[-2]) * \
                (self.k1k2[-1] + self.k1k2[-2])
            dmdt_out = ((self.__alpha1 * k1 + self.__alpha2 * k2)
                        * np.exp(-self.Integral))
            # sets values<0 =0.0, otherwise it will further cause
            # problem(nan)
            dmdt_out = np.where(abs(dmdt_out) > 1.e-300, dmdt_out, 0.0)
            return dmdt_out
    ...
    def ConvertKinFactors(self, ParameterVector):
        ...
        A1 = np.exp(-self.c * ParameterVector[0] / self.T_min +
                    self.c * ParameterVector[1] / self.T_max)
        E1 = self.c * ParameterVector[1] - self.c * ParameterVector[0]
        A2 = np.exp(-self.c * ParameterVector[2] / self.T_min +
                    self.c * ParameterVector[3] / self.T_max)
...
class DAEM(Model):
    ...
    def __init__(self, InitialParameterVector):
        ...
        self._x = np.array([np.sqrt((3 - np.sqrt(6)) / 2),
                            ...
                            np.sqrt((3 + np.sqrt(6)) / 2),
                            -np.sqrt((3 + np.sqrt(6)) / 2)])
        self._H = 8 * pow(self._x, 3) - 12 * \
            self._x  # Hermite polynomial
        from scipy.misc import factorial
        self._w = pow(2, self._nquad - 1) * factorial(self._nquad) * \
            np.sqrt(np.pi) / (pow(self._nquad, 2) * pow(self._H, 2))
    ...
    def calcMass(self, preProcResult, time, T, Name):
        ...
        for ti in range(len(timeInt)):
            if ti is 0:
                ...
                coeff3 = np.exp(-kint)
                ym = self._Wm * self._mT / \
                    np.sqrt(np.pi) * np.exp(-pow((Em - E0) /
                                                 sigma, 2) / 2) * np.exp(-kint)
                sumym = np.sum(ym)
