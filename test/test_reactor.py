...
def test_operating_conditions(reactor):
    ...
    np.testing.assert_array_equal(reactor.operating_conditions, operating_conditions)
...
def test_reactor_cpd():
    ...
    pressure = 5e5
    r = Reactor(
        model="CPD", ultimate_analysis=ua, pressure=pressure, name="test", max_step=1e-2
    )
    r.operating_conditions = operating_conditions
    ...
    np.testing.assert_allclose(sol[["char", "light_gas", "tar"]].sum(axis=1), 1)
