...
def test_postulate(runner):
    ...
    print(post_dict)
    mw = sum(post_dict["formula"][el] * mwi for el, mwi in M_elements.items())
    # print(mw)
...
def test_dulong(runner):
    runner.ultimate_analysis = {"C": 0.8, "H": 0.1, "O": 0.1, "S": 0, "N": 0}
    hhv = (333 * 80 + 1442 * (10 - 10 / 8.0)) * 1e3
...
def test_empirical(runner):
    ...
    gas = cantera.Solution(os.path.join(os.path.dirname(pkp.bins.__file__), "52.xml"))
    heat_vol = sum(
        val * runner.heat_of_reaction_species(sp) for sp, val in comp.items()
    )

    species = ["CH4", "O2", "CO2", "H2O"]
    hf = [gas.species(sp).thermo.h(273) for sp in species]
    coeff = [1, 2, -1, -2]
    deltaH = np.dot(hf, coeff) / gas.molecular_weights[gas.species_index("CH4")]
    ...
    heat_of_pyrolysis = (heat_vol - runner.lhv_daf) / (1 - comp["char"])
    assert np.isclose(heat_of_pyrolysis, runner.heat_of_pyrolysis(comp))

    # check composition == element balance
    sum_ua = sum(runner.ultimate_analysis.values()) - runner.ultimate_analysis["S"]
    print(sum_ua)
    print(runner.ultimate_analysis)
    for el in ("C", "H", "O", "N"):
        sum_c = 0
